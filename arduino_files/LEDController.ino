...
#define DELAYVAL_MOV 5
#define DELAYVAL_ANM 100

void callbackTest(){
  Serial.println("step complete");
}

NeoPattern pixels(NUMPIXELS, PIN, NEO_GRB + NEO_KHZ400, callbackTest);
  ...
  if (client) {                             // if you get a client,
    ...
    while (client.connected()) {            // loop while the client's connected
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (c == '\n' || c == '}') {                    // if the byte is a newline character
        ...
        if (currentLine.endsWith("GET /ping")) {
          Serial.println("\nPinged, sending pong");
        }
      }
    }
...
void anim_init() {
  ...
  for(int i = NUMPIXELS; i > 0; i--) {
    pixels.setPixelColor(i, 0, 255, 0);
    if(i != NUMPIXELS) { pixels.setPixelColor(i+1, 0, 0, 0); }
    pixels.show();
    delay(DELAYVAL_MOV);
  }
}

void anim_twoColorFade(uint32_t primary, uint32_t secondary) {
  Serial.println();
}

void alternatingFlash(int n) {
